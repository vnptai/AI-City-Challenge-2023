...
if __name__ == "__main__":
    ...
    for fold in args.folds:
        ...
        for image_id in list(np.unique(valid_df.image_id.values)):
            ...
            for _, row in tmp_df.iterrows():
                bbxs.append([float(row['xmin']),float(row['ymin']),float(row['xmax']),float(row['ymax']), row['label']])
        ...
        if args.network == 'fasterrcnn':
            ...
            model = get_effdet(args.backbone, num_classes=7, img_size=args.img_size, mode='valid', pretrained=False, pretrained_backbone=False)
            model = model.cuda()
            if args.use_amp:
        ...
        CHECKPOINT = 'checkpoints4/{}_{}_{}_fold{}.pth'.format(args.network, args.backbone, args.img_size, fold)
        ...
        for images, image_ids in tqdm(valid_loader):
            if args.network == 'fasterrcnn':
                ...
                for tta_transform in tta_transforms:
                    with torch.set_grad_enabled(False):
                        dets = model(tta_transform.effdet_augment(images.clone()), torch.tensor([1]*images.shape[0]).cuda())
    ...
    # results = []
    # for mt in mythreshs:
    #     ret = wbf_optimize(mt, ground_truth, box_pred, score_pred, label_pred)
    #     results.append(ret)
    ...
    for i in range(len(results)):
        if results[i]['all'][0] > max:
            idx_max = i
            max = results[i]['all'][0]

    # idx = np.argmax(np.array(results))
    print('-'*30)
    print('[Best box threshold]: {:.2f}'.format(mythreshs[idx_max].box_thresh))
    print('[Best nms threshold]: {:.2f}'.format(mythreshs[idx_max].nms_thresh))
    print('[Best pp threshold]: {:.2f}'.format(mythreshs[idx_max].pp_threshold))
    print('[AP]: {:.5f}'.format(results[idx_max]['all'][0]))
    print(results[idx_max])