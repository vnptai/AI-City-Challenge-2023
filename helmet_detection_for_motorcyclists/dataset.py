...
class AICityTrainDataset(Dataset):
    ...
    def refine_boxes(self, boxes, labels):
        result_boxes = []
        result_labels = []
    ...
    def resize_image(self, image, boxes, labels):
        cats = labels
        annotations = {'image': image, 'bboxes': boxes, 'category_id': cats}
        augmented = self.resize_transforms(**annotations)
        image = augmented['image']
        boxes = np.array(augmented['bboxes'])
        labels = np.array(augmented['category_id'])
        return image, boxes, labels

    def crop_image(self, image, boxes, xmin, ymin, xmax, ymax, labels):
        ...
        if len(result_boxes) > 0:
            ...
            result_labels = np.array(result_labels, dtype=float)
        else:
            result_boxes = np.array([], dtype=float).reshape(0, 4)
            result_labels = np.array([], dtype=float).reshape(0, 1)
        return image, result_boxes, result_labels

    def random_crop_resize(self, image, boxes, labels, img_size=1024, p=0.5):
        if random.random() > p:
            ...
            image, boxes, labels = self.crop_image(image, boxes, x, y, x + new_img_size, y + new_img_size, labels)
            return self.resize_image(image, boxes, labels)
        else:
    ...
    def load_image_and_boxes(self, image_path):
        ...
        boxes, labels = self.refine_boxes(boxes, labels)
        ...
        if len(boxes) > 0:
            ...
            labels = np.array([], dtype=float).reshape(0, 1)
    ...
    def load_cutmix_image_and_boxes(self, image_path, imsize=1024):     #custom mosaic data augmentation
        ...
        for i, img_path in enumerate(cutmix_image_paths):
            image, boxes, labels = self.load_image_and_boxes(img_path)
            if i == 0:
                ...
                image, boxes, labels = self.crop_image(image, boxes, 0, imsize-yc, imsize-xc, imsize, labels)
                ...
                image, boxes, labels = self.crop_image(image, boxes, 0, 0, imsize-xc, imsize-yc, labels)
                ...
                image, boxes, labels = self.crop_image(image, boxes, imsize-xc, 0, imsize, imsize-yc, labels)
        ...
        if self.mode == 'train':
            while(True):
                if random.random() > 0.5:
                    image, boxes, labels = self.load_image_and_boxes(image_path)
                else:
                    image, boxes, labels = self.load_cutmix_image_and_boxes(image_path)
                image, boxes, labels = self.random_crop_resize(image, boxes, labels, p=0.5)
                if len(boxes) > 0:
                    cats = labels
                    annotations = {'image': image, 'bboxes': boxes, 'category_id': cats}
...
class AICityDataset(Dataset):
    ...
    def load_image_and_boxes(self, image_id):
        ...
        if len(boxes) > 0:
            ...
            boxes = np.array([], dtype=float).reshape(0,4)
            labels = np.array([], dtype=float).reshape(0, 1)
        return img, boxes, source, labels
...
class AICityPseudoTestset(Dataset):
    ...
    def load_image_and_boxes(self, image_path):
        ...
        # if img.shape[0] != 1024 or img.shape[1] != 1024:
        augs = get_aug([
            Resize(height=1080, width=1920, interpolation=1, p=1)
        ])
        cats = labels
        annotations = {'image': img, 'bboxes': boxes, 'category_id': cats}
        augmented = augs(**annotations)
