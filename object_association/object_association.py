...
HUMAN_CLASSES = DRIVER_CLASSES + P1_CLASSES + P2_CLASSES

OVERLAP_60_RULE = False
REMOVE_SMALL_OBJS_RULE = True
REMOVE_MOTOR_WIHOUT_DRIVER = False  ## dang sai
DISPLAY = True
...
VIDEO_FOLDER = '/data/huytq/data/AIcitychallenge/track5/aicity2023_track5/aicity2023_track5_test/videos'
...
class Object_Association(object):
        ...
        for video_path in glob(os.path.join(self.video_folder, '*.mp4')):
            ...
            while video_cap.isOpened():
                ...
                if ret == True:
                    ...
                    if self.export_file:
                        write_enembled_results(self.export_file, results, video_id, frame_id, self.w, self.h, conf_thres=self.conf_thres)
    ...
    def foward_frame(self, frame, video_id, frame_id):
        ...
        if video_id in self.acity_labels.keys():
            ...
            # frame_humans = [human for human in frame_humans if human.motor_id]
