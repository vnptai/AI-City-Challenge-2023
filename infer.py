...
arg.add_argument('--video-folder', required=False,
                help='video-folder', default="/data/huytq/data/AIcitychallenge/track5/aicity2023_track5/aicity2023_track5_test/videos/")
args = arg.parse_args()

obj_association = Object_Association(video_folder=args.video_folder,
            head_thresh=0.14,
            display=False,
            prediction_path='./detection_results/pseudo_0.5_0.1_0.1.txt',
            head_label_path='./detection_results/effdet_ed7_768_head.txt')

conf_class_motor, conf_class_D, conf_class_D_No, conf_class_P1, cond_class_P1_No, conf_class_P2, conf_class_P2_No = 0.35, 0.32, 0.32, 0.32, 0.32, 0.2, 0.2 # v5 - pseudo_0.5_0.1_0.1- effdet_ed7_768_head - 0.7704 - best nick fake
...
for video_path in tqdm.tqdm(sorted(glob.glob(args.video_folder + "/*"))):
    ...
    tracking = tracker.my_tracking(max_age=3, min_hits=0, iou_threshold=0.15, enable_voting=False)
    ...
    while cap.isOpened():
        ...
        if len(bbox_human) > 0:
            ...
            for box in output_vehicle:
                cv2.rectangle(frame, (int(box[0]), int(box[1])), (int(box[2]), int(box[3])),
                            ...
                            2)
                x, y = box[0], box[1]
                text = "ID {} - {}-{}".format(int(box[6]), int(box[5]), round(box[4], 3))
        ...
        # cv2.imshow("image", cv2.resize(frame_rs, dsize=(960, 640)))
        # k = cv2.waitKey(1)
        # if k == 27:
        #     break
...
print(c_class_motor, c_class_D, c_class_D_No, c_class_P1, c_class_P1_No, c_class_P2, c_class_P2_No)
